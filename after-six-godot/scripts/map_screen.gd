## MapScreen — After Six: the same building, relit. The day game's cutaway with the lights
## off: a night sky far back, the floors in the dark, and a window lit only where someone
## is still in the building — the tag under each lit room says WHO (BMStrings who_*). That
## is the night's picture (beat_monday_map.md §1.1): progress is who is left in the dark.
##
## The day game's own header, kept for what it still describes:
## the building, drawn as a place: a sky plate far back, the building plate
...
func _ready() -> void:
	...
	if sp != "":
		...
		sky.modulate = Color(0.34, 0.26, 0.48)
	add_child(sky)
	# --- mid: the building -----------------------------------------------------------------
	building = Node2D.new()
	add_child(building)
	...
	if bp != "":
		...
		for f in bands.size():
			...
			s.modulate = Color(0.46, 0.38, 0.56)
		...
		if sp != "":
			...
			top.modulate = Color(0.3, 0.24, 0.44)
			building.add_child(top)
			building.move_child(top, 0)
	# the whole scene is dim; the lights bring the open floors up
	var cm := CanvasModulate.new()
	cm.color = Color(0.30, 0.22, 0.34)   # after six: the lights are off; the lit windows bring their rooms up
	add_child(cm)
	# --- the rooms ---------------------------------------------------------------------------
...
func refresh(profile: Dictionary) -> void:
	...
	for id in markers:
		...
		match st:
			"locked":
				# a dark window: nobody you can reach is in it yet
				m["win"].modulate = Color(0.16, 0.12, 0.2)
				m["name"].add_theme_color_override("font_color", Palette.DIM)
				m["tag"].text = BMStrings.t("locked")
				m["tag"].add_theme_color_override("font_color", Palette.FAINT)
				lights[id].enabled = false
			"open":
				# a lit window, and who is behind it
				m["win"].modulate = Color(1.0, 0.92, 0.96)
				...
				lights[id].color = Palette.ACCENT
				lights[id].energy = 1.3
			_:
				# done: the lamp left on, gold, and nobody in it any more
				m["win"].modulate = Color(0.9, 0.82, 0.7)
