...
# A parse error in one class silently skips every check that used it (it happened: the
# first run reported 90 passed with idle.gd not compiled), so the run is only green when
# the engine printed no errors at all.
set -uo pipefail
cd "$(dirname "$0")/.."
GODOT=${GODOT:-$HOME/bin/godot/Godot_v4.7-stable_linux.x86_64}
OUT=$(timeout 300 "$GODOT" --headless --path . res://tests/run_tests.tscn 2>&1)
CODE=$?
echo "$OUT" | grep -v '^\s*$'
