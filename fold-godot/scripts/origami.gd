## Origami — which model a level folds, and which fold the board is on.
##
## ops/fold/BRIDGE.md is the argument; this is the lookup it needs. The whole bridge rests
## on one fact that was already true and never said out loud: **the number on a tile is
## the number of paper layers**, and a sheet folded k times is 2^k layers thick. So a 2 is
## one fold, a 4 is two, a 256 is eight, and the level's `target` is the fold that
## finishes the model.
##
## Nothing in here is a rule. `Fold` decides what happens on the board and its conformance
## test pins it to the shipped JavaScript; this file only names what the player is doing.
## If every line of this file were deleted the game would still play identically — it
## would just go back to being a merge puzzle called FOLD that never folds anything.
##
## Two data files, both already on disk:
##   data/models.json — level -> model, for all 201 levels
##   data/steps.json  — model -> the real diagram steps, each with the layer count `at`
##                      that it reaches
##
## Autoloaded as "Origami".
extends Node
...
var _models := {}        # model key -> {zh, en, steps:[{at, zh, en}]}
...
## The diagram step a tile of this value has just completed — "Petal-folded up — the bird
## base" — or {} when this value is not a step of this model. The steps do not start at 2:
## the first one is `at: 4`, because a single fold of a square is not yet a shape with a
## name. A 2 on the board is therefore mid-step and returns {}.
