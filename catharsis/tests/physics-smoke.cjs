...
const physicsPath = path.join(__dirname, "../kernel/physics.js");
vm.runInContext(
...
const { WORLD, launch, updatePhysics, settleBalls, setFlipper, updateParticles } = context.testApi;
...
console.log(`kernel physics ok: unpressed=${unpressedHits}, spread=${JSON.stringify(results)}, skillHits=${rallyContacts}, saves=${saves}`);
