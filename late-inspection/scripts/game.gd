...
func _advance(next_stage: int, obj_key: String, next_chapter: int, clock: String) -> void:
	...
	if next_chapter >= 2 and next_chapter > chapter_index:   # Chapters I-II are the advertised free slice; the gate opens on Chapter III
		Gate.block("ch%d" % (next_chapter + 1), _chapter(next_chapter))   # dual-track (ops/DUAL_TRACK.md)
	stage = next_stage
...
func _finish(id: String) -> void:
	...
	# games as links the player may click. The mouse was released above. Casual board only —
	# mainstream title.
