...
func _init() -> void:
	...
	# six minutes traversal/composition. This excludes pause/idling.
	var expected_seconds := int(float(words) / 170.0 * 60.0) + pages * 4 + notes * 5 + choices * 15 + 360
	if words < 2000 or pages < 60 or notes < 20 or choices != 4 or expected_seconds < 1500 or expected_seconds > 2100:
