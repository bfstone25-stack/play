...
def _load_faces(size: int) -> dict[str, ImageFont.FreeTypeFont]:
    ...
    if "wqy" not in faces:
        raise SystemExit("WenQuanYi Micro Hei is required to build Floor 13 BMFont")
    return faces
...
def build(size: int, chars: list[str]) -> tuple[int, tuple[int, int], int]:
    ...
    stem = f"floor13_pixel_{size}"
    atlas_name = f"{stem}.png"
    atlas.save(FONT_DIR / atlas_name, optimize=True)

    line_height = size + LINE_PAD
    lines = [
        f'info face="Floor13 Pixel {size}" size={size} bold=0 italic=0 charset="" unicode=1 stretchH=100 smooth=0 aa=1 padding=0,0,0,0 spacing=0,0 outline=0',
