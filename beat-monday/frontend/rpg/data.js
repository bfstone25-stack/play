/* Beat Monday RPG — all numbers live here. Adding content = adding rows, never code.
 * Extension seams: DAYS (a week), WEEKS (more weeks), ROLES (classes), EQUIP/SKILLS tables.
 */
var BMData = (() => {
  // ---- roles (classes). One shipped; the table is the seam for more. ----
  const ROLES = [
    { id: "ic", base: { hp: 100, atk: 10, rate: 2.2, spd: 150 }, growth: { hp: 8, atk: 1.2 } },
  ];

  // ---- enemies: office nuisances ----
  const FOES = {
  ...
  // ---- five days, five antagonists. A day is one level. ----
  const DAYS = [
  ...
  // ---- weeks: the length seam. Week 1 ships; later weeks re-run DAYS with modifiers. ----
  const WEEKS = [
    { id: "w1", hpMul: 1, dmgMul: 1 },
    { id: "w2", hpMul: 1.6, dmgMul: 1.25 },   // crunch week
    { id: "w3", hpMul: 2.4, dmgMul: 1.5 },    // reorg
  ];

  // ---- equipment: office objects. Three slots. Boss drops. ----
  const EQUIP = [
  ...
  // ---- party: colleagues recruited by clearing a day. Orbit and chip in. ----
  const PARTY = [
  ...
  // ---- rant catalog: ids the kernel rant.js scores. Text comes from BMPhrases. ----
  const RANT = [
    { id: "ok",    power: 10 },
    { id: "lie",   power: 25 },
    { id: "teach", power: 40 },
  ...
  return { ROLES, FOES, DAYS, WEEKS, EQUIP, SKILLS, PARTY, RANT };
})();
if (typeof module !== "undefined") module.exports = BMData;
