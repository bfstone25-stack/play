...
def attach_block(db, pid, route, history, message, zh):
    ...
    if det["type"] != "unknown" and det["confidence"] >= 0.4 and \
    ...
    eff_type = stored_type if stored_type and stored_conf >= 0.4 else "unknown"
    return _attach.strategy(eff_type, zh), {"type": eff_type, "confidence": round(stored_conf, 2)}

def _chat(system, messages, max_tokens=240, temperature=0.85):
    _claim_flutter_gpu(wait=True)
...
def _claim_flutter_gpu(wait=False):
    """Refresh Flutter's lease through the shared all-product scheduler."""
    try:
        result = subprocess.run([os.path.expanduser("~/bin/llm_claim.sh"), "flutter", "8807"],
...
def say(r: SayReq):
    ...
    if _story and _story.has_story(r.route):
        ...
        if _judge_ch is not None:
            try:
                ...
                _jraw = _chat(_jsys, [{"role": "user", "content": _jconvo}], max_tokens=60, temperature=0.2)
                _passed, _reason = _story.parse_judge(_jraw)
                if _passed:
                    _story_state = _story.mark_goal_cleared(_story_state, _judge_ch["id"])
                else:
                    # 目标没达成: 好感够但不放行,给前端一句"还差什么"的软提示
                    _goal_verdict = {"passed": False, "reason": _reason,
    ...
    reply = _sanitize_market_output(
        _chat(sysp, r.history[-10:] + [{"role": "user", "content": msg}]), r.lang)
    _signals, _heart = interaction_signals(msg, content_lang, r.history, r.route)
    new_aff = max(0, min(r.affection + _heart, 100))
    mood = MOOD[new_aff % len(MOOD)]

    # 事实级记忆抽取: 每6轮抽0-2条原子事实入库(服务端持久)
    mem_update = ""
    if len(r.history) >= 4 and len(r.history) % 6 == 0:
        try:
            raw = _chat(
                ...
                r.history[-6:] + [{"role": "user", "content": msg}], max_tokens=80, temperature=0.2)
            m = re.search(r"\[.*\]", raw, re.S)
            new_facts = json.loads(m.group(0)) if m else []
            for fa in new_facts[:2]:
                if isinstance(fa, str) and 3 < len(fa) < 120:
                    db.execute("INSERT INTO memories(pid,route,fact,ts) VALUES(?,?,?,?)",
                               (pid, r.route, fa.strip(), time.time()))
            if new_facts: mem_update = "; ".join(str(x) for x in new_facts[:2])
        except Exception:
            mem_update = ""
