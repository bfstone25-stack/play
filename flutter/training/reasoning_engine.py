...
INTENT_TERMS = {
    "boundary": ("别碰", "别这样", "不舒服", "停下", "go away", "stop", "don't touch", "やめて", "触らない", "déjame", "pare", "não me toque"),
    ...
    "factual": ("谁是", "多少", "哪本书", "是真的吗", "里有", "作者", "谁写的", "多少回", "who is", "how many", "is it true", "author", "誰", "何巻", "作者", "quién", "quantos"),
    "preference": ("喜欢什么", "最喜欢", "不喜欢", "favorite", "prefer", "好きな", "好み", "favorito", "prefere"),
    "deep_talk": ("为什么", "你觉得", "梦想", "童年", "害怕什么", "意义", "why", "what do you think", "dream", "childhood", "meaning", "どう思う", "夢", "por qué", "sonho"),
...
def verify_candidate(reply: str, market: str, recent_replies, decision: Decision, source_message: str = ""):
    ...
    contract_terms = {
        ...
        "factual": {
            ...
            "es": ("seguro", "verificar", "comprobar", "correcto", "no sé"), "pt": ("certeza", "verificar", "conferir", "correto", "não sei")},
        "distress": {
...
def safe_fallback(market: str, decision: Decision) -> str:
    pack = {
        "zh": {
        ...
        "ja": {"factual":"それは確信がありません。何を指しているのか、もう少し教えてくれますか。","boundary":"わかった。ここで止める。君の境界は尊重する。","distress":"今、本当につらいんだね。急いで答えを出さなくていい。いちばん苦しいことを話せる？","commitment":"うん、その約束は覚えておく。時間や場所が変わったら教えて。","general":"今の言葉をきちんと受け取りたい。もう一度聞かせてくれる？"},
        "en": {"factual":"I'm not certain enough to pretend. Which work or event did you mean?","boundary":"Okay. I'll stop. I hear your boundary.","distress":"That sounds genuinely hard. I won't rush to fix it—what part is weighing on you most?","commitment":"All right. I'll remember the plan. Tell me if the time or place changes.","general":"I don't think I caught that properly. Will you say it again?"},
        "es": {"factual":"No estoy lo bastante seguro para inventarlo. ¿A qué obra o hecho te refieres?","boundary":"De acuerdo. Me detengo. Entiendo tu límite.","distress":"Eso suena realmente difícil. No voy a apresurarme a arreglarlo; ¿qué es lo que más te pesa?","commitment":"De acuerdo, recordaré el plan. Avísame si cambia la hora o el lugar.","general":"Creo que no lo entendí bien. ¿Me lo dices otra vez?"},
        "pt": {"factual":"Não tenho certeza suficiente para inventar. A qual obra ou situação você se refere?","boundary":"Tudo bem. Vou parar. Entendi o seu limite.","distress":"Isso parece realmente difícil. Não vou tentar consertar tudo às pressas; o que está pesando mais?","commitment":"Certo, vou lembrar do combinado. Me avise se o horário ou o lugar mudar.","general":"Acho que não entendi direito. Pode me dizer de novo?"},
    }
    return pack[market].get(decision.intent, pack[market]["general"])
