...
# 参数可调: 帧数/FreeInit迭代数
FRAMES = int(sys.argv[1]) if len(sys.argv)>1 else 48
FI_ITER = int(sys.argv[2]) if len(sys.argv)>2 else 2
...
# 剧情Prompt Travel(惊恐→掏宝典得意). 身型描述统一(防走样)
BODY='a cute funny cartoon monster, consistent body shape, blue helmet, round yellow belly, thin red legs, black shoes'
PT = f'"0": "{BODY}, shocked terrified face, mouth open, arms up",\n"24": "{BODY}, smug confident grin, one hand on hip, proud"'
...
W = {
 ...
 # 第3招 FreeInit: gaussian低频滤波,消抖稳身型
 ...
 # 第2招: weight 0.7, end_at 0.7(前70%锁脸后30%放开肢体)
 ...
 '13':{'class_type':'VHS_VideoCombine','inputs':{'images':['12',0],'frame_rate':16,'loop_count':0,'filename_prefix':'dupang_v2','format':'video/h264-mp4','pingpong':False,'save_output':True}},
}
