...
const PATH := "user://fold_after_dark_save.json"
...
## Generic slots for the fork's own state (scenes unlocked, streaks, the daily stub).
func get_v(key: String, default):
	return _data.get(key, default)


func set_v(key: String, v) -> void:
	_data[key] = v
	flush()
