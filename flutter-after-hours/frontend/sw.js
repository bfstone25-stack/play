"use strict";
const CACHE="flutter-pwa-20260817071322", CORE=["./","./index.html","./pwa-install.js","./manifest.webmanifest","./icon-192-v2.png","./icon-512-v2.png"];
self.addEventListener("install",e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(CORE)).then(()=>self.skipWaiting())));
self.addEventListener("activate",e=>e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k.startsWith("flutter-pwa-")&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
