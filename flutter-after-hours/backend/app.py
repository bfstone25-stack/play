...
MILESTONES = {10: ("初识", "First spark"), 30: ("心动", "Heartbeat"),
              60: ("交心", "Trust"), 100: ("怦然", "Fallen for you")}


def _milestone(old_aff, new_aff, zh):
    """Fire on the crossing, not on the landing.

    The parent fires `new_aff in (10,30,60,100)`, which a +5 turn jumps clean
    over. That is survivable when a milestone is only confetti; it is a bug once
    a CG hangs off the 60 mark, so the fork tests the crossing instead.
    """
    for th in (100, 60, 30, 10):
        if old_aff < th <= new_aff:
            return MILESTONES[th][0] if zh else MILESTONES[th][1]
    return None


def _static_say(r):
    """One authored turn. No model call, no GPU claim, no judge pass.

    Affection still moves through the parent's zero-call `interaction_signals()`
    and the attachment lane still comes from `attachment.detect()`, so the two
    things that decide the branching are exactly the ones that were already free.
    """
    ...
    lane = (_attach_info or {}).get("type", "unknown")

    st_obj = _story.load_story(r.route)
    reply, event, chap, choice, state, cg = _sx.turn(
        st_obj, r.story_state, r.affection, new_aff, lane, content_lang, len(r.history))

    prog = _story.story_progress(st_obj, state, new_aff)
    if choice and chap:
        prog["choice"] = {
            "id": choice["id"], "chapter": chap["id"],
            "prompt": _field(choice, "prompt", content_lang),
            "options": [{"text": _field(o, "text", content_lang), "index": i}
                        for i, o in enumerate(choice.get("options", []))]}

    log_signals(db, pid, r.route, r.lang, state, msg, _signals, _heart)
    db.execute("INSERT INTO distill_log VALUES(?,?,?,?,?,?)",
               (time.time(), r.route, pid, msg, reply, new_aff))
    db.commit(); db.close()

    out = {"reply": reply, "affection": new_aff, "mood": MOOD[new_aff % len(MOOD)],
           "heart_gain": _heart, "signals": _signals, "memory_update": "",
           "milestone": _milestone(r.affection, new_aff, zh),
           "story": prog, "story_state": state, "static": True}
    if event:
        out["beat"] = event
    if _attach_info and _attach_info["type"] != "unknown":
        out["attach"] = _attach_info
    if cg:
        # CG hook 2 of 3 — affection crossed 60. The client reveals it through
        # the same Gate that already guards the chapters (index.html).
        out["cg"] = cg
    return out
...
def say_stream(r: SayReq):
    ...
    # Static route: nothing is being generated, but the client's streaming path
    # stays the one code path. The authored line is chunked so the typing
    # animation still reads as speech rather than as a paste.
    if _story and _sx.is_static(r.route):
        done = _static_say(r)
        ...
        def gen_static():
            ...
            for i in range(0, len(text), 24):
                yield f"data: {json.dumps({'token': text[i:i+24]})}\n\n"
            d = dict(done); d["done"] = True
...
class ChooseReq(BaseModel):
    ...
    story_state: dict = {}
    lang: str = "en"
    lane: str = ""          # attachment lane, echoed back by the client
...
def choose(r: ChooseReq):
    ...
    if _static:
        ...
        _cs = next((c for c in (_ch or {}).get("choices", []) if c["id"] == r.choice), None)
        _opts = (_cs or {}).get("options", [])
        if _opts and r.option < len(_opts):
            reply = _sx.choice_reply(_opts[r.option], r.lane, _content_lang(r.lang)) or reply
    # Legacy EN stories authored their choice replies in ZH only, and the parent
    # translates them at runtime — which is a model call, on the GPU, inside a
    # button press. A static route is authored in its own language, so it must
    # never take that path: it is the one place a "no-LLM" fork would still have
    # blocked on a contended llama-server.
    if _content_lang(r.lang) == "en" and not _static:
    ...
    out = {"affection": new_aff, "reply": reply, "story_state": new_state,
           "aff_delta": d_aff}
    # CG hook 2 of 3, second entry point. A chapter-end choice carries its own
    # affection delta, so the 60 mark is just as likely to be crossed here as in
    # a /say turn — and the first HTTP playthrough of this fork crossed it here
    # on two of the three routes. Watching only /say loses the plate silently.
    if _static and r.affection < 60 <= new_aff and st_obj.get("cg_heat"):
        out["cg"] = st_obj["cg_heat"]
    # 是否到最终章 → 判定结局
    chs = st_obj["chapters"]
    if new_state.get("chapter") == chs[-1]["id"] and new_state.get("choice_done_" + chs[-1]["id"]):
        ...
        if end:
            ...
            # CG hook 3 of 3 — earned by match_ending(), never by playtime.
            # An ending without a `cg` (the bittersweet fallback) shows none.
            if end.get("cg"):
                out["ending"]["cg"] = end["cg"]
        ...
        out["chapter_clear"] = {
            ...
            "reward": {
                ...
                "title": title,
                "text": reply,
                # CG hook 1 of 3 — a chapter carries a `cg` slot or it does not.
                # Nothing here is unlocked by time spent in the game.
                "cg": _sx.chapter_cg(st_obj, r.chapter),
            },
...
@app.get("/cg/{route}")
def cg_manifest(route: str):
    """Every CG slot this route can produce, and how each one is earned.

    The gallery renders locked plates from this, so a player can see the shape
    of what is missing without any of it being reachable from a menu.
    """
    if not _story or not _story.has_story(route):
        return {"slots": []}
    st = _story.load_story(route)
    ...
    for c in st.get("chapters", []):
        if c.get("cg"):
            earned[c["cg"]] = {"how": "chapter_clear", "chapter": c["id"]}
    if st.get("cg_heat"):
        earned[st["cg_heat"]] = {"how": "affection", "at": 60}
    for e in st.get("endings", []):
        if e.get("cg"):
            earned.setdefault(e["cg"], {"how": "ending", "ending": e["id"]})
    return {"route": route, "slots": [dict(slot=k, **v) for k, v in earned.items()]}
