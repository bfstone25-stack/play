...
// Overtime Landlord: four of the eight symbols are the people who work in the building.
// Payouts and tags are byte-identical to the SFW catalogue, so every adjacency rule and the
// whole rent curve are unchanged — only the nouns moved.
//   dan   (was "dev")      staff 3  the engineer the coffee chain runs through
//   priya (was "intern")   staff 1  covers whoever she is standing next to
//   mara  (was "standup")  staff 1  night manager; lifts every neighbour
//   wes   (was "meeting")  noise 2  the tenant on 7 whose "meetings" tax the staff
...
// The four placeable people. Used by the CG predicates and the floor-9 lead pick.
const LANDLORD_CAST = ["mara", "dan", "priya", "wes"];
...
function settleGrid(cells, catalog, relics) {
  ...
  const staff = new Set(["dan", "priya", "mara"]);
  ...
  cells.forEach((id, i) => {
    ...
    if (id === "priya") {
      ...
      neighborsOf(i).forEach((n) => {
        const oid = cells[n];
        if (!oid || oid === "priya") return;
        const ob = rawBase(oid, n, byId);
        if (ob > best) { best = ob; from = n; }
      });
      base = best;
      if (from >= 0) note("priya-copy", i, from);
    }
    if (id === "printer") {
  ...
  cells.forEach((id, i) => {
    ...
    neighborsOf(i).forEach((n) => {
      ...
      if (!other) return;
      if (id === "coffee" && oid === "dan") {
        cellMult[i] *= 3;
        note("coffee-dan", i, n);
      }
      if (id === "dan" && oid === "coffee") {
        cellMult[i] *= 1;
      }
      if (id === "dan" && oid === "mute") {
        cellAdd[i] += 2;
  ...
  if (!has("glass")) {
    cells.forEach((id, i) => {
      ...
      neighborsOf(i).forEach((n) => {
        const oid = cells[n];
        if ((oid === "priya" || oid === "dan") && !shielded[n]) {
          cellTax[n] += 1;
          note("wes-tax", i, n);
        }
      });
  ...
  if (has("army")) {
    ...
    cells.forEach((id, i) => {
      ...
      let from = -1;
      neighborsOf(i).forEach((n) => {
        if (cells[n] && cells[n] !== "priya" && pre[n] > best) {
          best = pre[n];
          from = n;
        }
      });
      cellScore[i] = best;
      if (from >= 0) note("priya-army", i, from);
    });
  }

  let payout = cellScore.reduce((a, b) => a + b, 0);
  if (has("badge")) {
    const chain = events.filter((e) => e !== "wes-tax").length;
    payout += chain;
  }
