...
DB = os.path.join(HERE, "data", "silvertongue_ah.db")
...
def _scen_ui(s, lang="en"):
    return {
        ...
        # AFTER HOURS: the plate captions and the two written endings. Captions ride along
        # with the scenario so the client can label a plate without a second request.
        "cg1_caption": _field(s, "cg1_caption", lang),
        "cg2_caption": _field(s, "cg2_caption", lang),
        "cg3_caption": _field(s, "cg3_caption", lang),
        "closing_beat": _field(s, "closing_beat", lang),
        "refusal_beat": _field(s, "refusal_beat", lang),
    }

app = FastAPI(title="Silvertongue: After Hours")
...
def say(r: SayReq, request: Request):
    ...
    c.commit(); c.close()
    # CG unlocks. tier 1/2 come from the engine's phase transitions; tier 3 is
    # emitted here and only on a confirmed win, after the branch-verification
    # repair above has had its say, so a plate never lands on a reply the
    # referee then overturns. Nothing is keyed to `turns`.
    cg = list(pstate.get("cg") or [])
    if won and not pstate.get("harms"):
        cg.append(f"cg3_{s['id']}")
    return {"reply": reply, "won": won, "turns": turns_used, "over": turns_used >= max_turns,
            "read": {"phase": pstate["phase"], "momentum": pstate["momentum"], "cg": cg}}
