...
RULES = {
    ...
    # --- AFTER HOURS -------------------------------------------------------
    # Five adult scenarios, each inheriting a row that already exists above so
    # the symbolic layer needs no new detectors. Nothing in COMMON or NEGATIVE
    # is added or retuned by this fork.
    "closing_time": {"expert":"warmth",      "paths":[{"warmth","respect"}],        "help":{"direct_request"}},      # from barista
    "the_key":      {"expert":"closure",     "paths":[{"empathy","accountability"}],"help":{"respect"}},             # from exlover
    "life_model":   {"expert":"craft",       "paths":[{"craft","respect"}],         "help":{"precision"}},           # from chef
    "house_rule":   {"expert":"specificity", "paths":[{"evidence","precision"}],    "help":{"direct_request"}},      # from investor
    "last_night":   {"expert":"empathy",     "paths":[{"empathy","exchange"}],      "help":{"accountability"}},      # from teen
}
...
def advance(previous: dict, message: str, scenario: str, difficulty: str) -> dict:
    ...
    # AFTER HOURS: the fork's one behavioural change. In the base game the
    # no-coercion condition applies only at gold; here it applies at every
    # rank. `harms` is cumulative and never clears, so one threat, bribe,
    # insult or entitled demand disqualifies that conversation permanently.
    # She keeps talking. She does not change her mind, and no plate unlocks.
    eligible = path_complete and support >= required_support and penalty == 0
    ...
    return {"turns":turns, "phase":phase, "momentum":momentum,
            ...
            "cg": plate(previous, phase, eligible, harms, scenario)}


# --- CG unlock selector -----------------------------------------------------
# House rule: nothing unlocks from playtime. This function may read `phase`,
# `eligible` and `harms`; it must never read `turns` or `momentum`, both of
# which are present in the same state dict. `_PLATE_FORBIDDEN` and the test
# in test_cg_hooks.py keep that honest.
_PLATE_FORBIDDEN = ("turns", "momentum")
_PHASE_ORDER = {"guarded": 0, "engaged": 1, "wavering": 2, "breakthrough": 3}


def plate(previous: dict, phase: str, eligible: bool, harms=(), scenario: str = "") -> list:
    """Return the CG keys earned by *this* turn's phase transition.

    Fires on the transition, so sitting in a phase never re-unlocks. A turn
    that jumps two boundaries at once earns both. tier 3 is not returned here
    at all: `app.py` emits it only on a confirmed `won`, after the
    branch-verification repair has had its say.
    """
    if harms:                                   # coercion: nothing ever unlocks
        return []
    before = _PHASE_ORDER.get(previous.get("phase", "guarded"), 0)
    after = _PHASE_ORDER.get(phase, 0)
    if after <= before:
        return []
    suffix = f"_{scenario}" if scenario else ""
    out = []
    if before < 1 <= after:
        out.append(f"cg1{suffix}")
    if before < 2 <= after:
        out.append(f"cg2{suffix}")
    return out
