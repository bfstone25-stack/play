...
(() => {
  ...
  function useNudge(dir) {
    if (phaseNudge(dir)) {
      CAM.punch = 10;
      CAM.aberration = 0.7;
      toast(dir < 0 ? "LEFT BEND" : "RIGHT BEND");
      mascotSay(dir < 0 ? "命运向左折了一下。" : "从裂谷上飞过去。", "◇");
      SFX.impact();
    } else if (WORLD.balls.length) toast("偏转已经用过了");
    else toast("先把光打出去");
  }
  $("nudgeLeft").addEventListener("click", () => useNudge(-1));
  $("nudgeRight").addEventListener("click", () => useNudge(1));
  ...
  window.addEventListener("keydown", e => {
    ...
    if (e.code === "ArrowLeft" || e.code === "KeyA") useNudge(-1);
    if (e.code === "ArrowRight" || e.code === "KeyD") useNudge(1);
  });
  ...
  function loop(now) {
    ...
    updateParticles(dt);
    syncCamera();
    try { drawMachine(); } catch (err) { console.error(err); }
