...
const LAYOUT={width:420,height:640,ballR:8,pegR:4,gravity:1120,restitution:.76,wallBounce:.82,topBounce:.75,maxBalls:4,leftWall:18,rightWall:402,pocketTop:548,pocketH:56,pocketCount:7,launcherX:210,launcherY:614,launchPowerMax:1250};
...
function hitNode(ball,n){
  ...
  if(vn<0){const boost=n.type==="bumper"?1.35:1;const j=-(1+LAYOUT.restitution)*vn*boost;ball.vx+=j*nx;ball.vy+=j*ny;const tangent=(n.type==="switch"?.16:.07)*(Math.random()-.5)*Math.hypot(ball.vx,ball.vy);ball.vx+=-ny*tangent;ball.vy+=nx*tangent;SFX.peg();if(typeof onResonanceHit==="function")onResonanceHit(n);}
  n.lit=1;
...
function burstParticles(x,y,n,color){const low=!!window.__NS_LOWFX;const count=low?Math.min(8,Math.ceil(n*0.35)):n;for(let i=0;i<count;i++){const a=Math.random()*Math.PI*2,sp=60+Math.random()*230;WORLD.particles.push({x,y,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-40,life:.4+Math.random()*.4,maxLife:.9,color,size:1.5+Math.random()*3})}const cap=low?36:120;if(WORLD.particles.length>cap)WORLD.particles.splice(0,WORLD.particles.length-cap)}
