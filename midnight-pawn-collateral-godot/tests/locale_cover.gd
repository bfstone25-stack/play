...
## A source with nothing to translate once the brand, the bbcode tags and the
## placeholders are removed ("FLAT 404  ·  blazeCore Play", "[color=#c8bfd0]%s[/color]")
## is allowed to come back identical.
func _needs_translation(s: String) -> bool:
	var r := RegEx.new()
	r.compile("\\[[^\\]]*\\]|%\\d*[ds]|FLAT 404|blazeCore Play|LIEN|COLLATERAL")
	var rest := r.sub(s, "", true)
	var w := RegEx.new()
	w.compile("[A-Za-z]{2,}")
	return w.search(rest) != null
