...
bash tests/run.sh                        # 87 headless checks; fails on any engine error
...
- `tests/run.sh` — 87 checks: 648 steps of clicks/ticks/upgrades/loads identical to
...
- `tests/headless_web.py` — the real web export in headless Chromium through the whole
  ...
  into the same collection; all four forces revealing the predicted answer, each arriving
  through the one rhythm (a half-line while she goes quiet, the dim-and-hold, the line); the shop
