...
init -1 python:
    ...
    _DEFAULT_TEL_URL = "https://apps.blazecore.dev/tel_batch?app=collateral"
    _TEL_APP = "collateral"
    _TEL_FLUSH_AT = 6
    _TEL_TIMEOUT_S = 4.0
    ...
    if persistent.telemetry_endpoint is None or "127.0.0.1:27100" in (persistent.telemetry_endpoint or ""):
        persistent.telemetry_endpoint = os.environ.get("COLLATERAL_TELEMETRY_URL", _DEFAULT_TEL_URL)
    ...
    # Tracks that promised to call nothing: the downloadable trial, the DLsite demo, and
    # any portal build. Stripping the page-level SDK was not enough — this module carries
    # its own endpoint, so a build that claims to call nothing still did. Decided here,
    # before anything can send. (Found on Elena 2026-09-17 via a Newgrounds player.)
    _TEL_SILENT_TRACKS = ("offline_ads", "demo")
    try:
        if getattr(renpy, "emscripten", False):
            import emscripten
            _tel_track_name = emscripten.run_script_string("(window.COLLATERAL_DIST || '')")
        else:
            _tel_track_name = (renpy.file("dist.txt").read().decode("utf-8").strip()
                               if renpy.loadable("dist.txt") else "paid")
    except Exception:
        _tel_track_name = ""
    if _tel_track_name in _TEL_SILENT_TRACKS:
        persistent.telemetry_endpoint = ""
    ...
    if persistent.telemetry_pid is None:
        persistent.telemetry_pid = "r" + uuid.uuid4().hex[:12]
    ...
    def tel_ensure_session():
        ...
        tel_track(
            "session_start",
            {
                ...
                "dist": dist_track() if "dist_track" in globals() else "",
            },
        )
        tel_flush(force=True)
