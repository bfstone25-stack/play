...
##   itch_web  itch.io browser play: cold open + 2 appraisals free, climax CGs censored, then the $3.49 gate.
##   ads_web   Cloudflare Pages: every unlock — next chapter, uncensored CG — costs one
##             sponsor clip. The page's index.html sets window.COLLATERAL_DIST and loads
##             web/collateral_ads.js, which draws the ad overlay and reports back.
##
...
init -2 python:
    # Five of the six readings are censored before unlocking. `tamsin` — the free tutorial
    # reading — is deliberately not in here: one genuinely uncensored plate up front is what
    # makes the rest legible as a purchase rather than as a bait screen. (Room 704 gated both
    # of its CGs and 75 browser plays bought nothing.)
    ...
    def dist_track():
        if not getattr(renpy, "emscripten", False):
            ...
            if getattr(store, "_mpc_dist", None) is None:
                try:
                    store._mpc_dist = renpy.file("dist.txt").read().decode("utf-8").strip() if renpy.loadable("dist.txt") else "paid"
                except Exception:
                    store._mpc_dist = "paid"
                ...
                if store._mpc_dist not in ("paid", "offline_ads", "demo"):
                    store._mpc_dist = "paid"
            return store._mpc_dist
        if getattr(store, "_mpc_dist", None) is None:
            d = _js_str("(window.COLLATERAL_DIST || 'itch_web')")
            store._mpc_dist = d if d in ("itch_web", "ads_web", "paid") else "itch_web"
        return store._mpc_dist
    ...
    def gated_ticket(name):
        ...
        try:
            d = _http_json(UNLOCK_API + "/unlock/start", {"app": "collateral", "key": name})
            if d and d.get("ok"):
    ...
    def gated_redeem(name):
        ...
        try:
            url = "%s/unlock/fetch?ticket=%s&app=collateral&key=%s" % (UNLOCK_API, tk["ticket"], name)
    ...
    def ad_gate_open(key, kind):
        ...
        tel_flush(True)
        _js("window.CollateralAds && window.CollateralAds.open(%r, %r)" % (key, kind))

    AD_SCRIPT_GRACE = 8   # seconds to wait for collateral_ads.js before calling the sponsor unavailable
    ...
    def ad_gate_poll(started=None):
        ...
        import time
        if not _js_int("window.CollateralAds ? 1 : 0"):
            if started is not None and time.time() - started >= AD_SCRIPT_GRACE + DIRECT_LINK_SECONDS:
                ...
                return 3
            return 0
        return _js_int("window.CollateralAds.result()")
    ...
    def direct_link_url():
        ...
        if dist_track() != "ads_web" or not getattr(renpy, "emscripten", False):
            return ""
        return _js_str("(window.COLLATERAL_DIRECT_LINK || '')")
...
## Watch-to-unlock overlay. The ad itself is HTML drawn over the canvas by collateral_ads.js;
    ...
    frame:
        ...
        vbox:
            ...
            hbox:
                ...
                textbutton _("★ Unlock everything — $3.49"):
                ...
                if renpy.emscripten and dist_track() != "demo":
                    ...
                    textbutton _("▶ Unlock free with ads — on our site"):
                        action [Function(cg_buy_decide, name, "ads_click", t0), OpenURL("https://midnight-pawn-collateral.flat404.workers.dev/?src=itch")]
                        text_size 22 text_color "#70c080" background Transform("#1a2a1e", alpha=0.95) padding (18, 12, 18, 12)
                else:
                    textbutton _("▶ Free with ads"):
                        action [Function(cg_buy_decide, name, "ads_click", t0), OpenURL("https://midnight-pawn-collateral.flat404.workers.dev/?src=ingame")]
                        text_size 22 text_color "#70c080" background Transform("#1a2a1e", alpha=0.95) padding (18, 12, 18, 12)
    ...
    menu:
        ...
        "Open the sponsor page in a new tab instead" if direct_link_available():
            ...
            if _return == 1:
                return
            jump chapter_gate_retry
        "Get the full game instead ($3.49, no ads)":
...
label warp_gate:
    $ new_run()
    scene bg shop
    nar "[[warp] Reading fee, then the CG gate."
    call reading_offer("ring") from _call_warp_reading
    nar "[[warp] Till is [run.till]. Chapter gate next."
    $ act_cleared = 1
    $ demo_cut = "warp"
    call chapter_gate(2) from _call_warp_chapter
    nar "[[warp] Appraisal 3 unlocked."
    jump act_veil
...
## F95 download: the next chapter costs one sponsor page (or the $3.49 itch build).
...
screen offline_trial_screen():
    ...
    vbox:
        ...
        textbutton _("▶ Keep playing free, with ads - in your browser"):
            action [Function(tel_cta_click, "trial_ads_site"), OpenURL("https://midnight-pawn-collateral.flat404.workers.dev/?src=trial")]
            xalign 0.5 text_size 24 text_color "#ffffff" background Transform("#1f4a2a", alpha=0.95) padding (22, 12, 22, 12)
        textbutton _("★ Get the full game, no ads - $3.49 on itch"):
...
label checkpoint_clip:
    ...
    menu:
        ...
        "Play the clip":
            jump checkpoint_clip
        "Get the ad-free full game ($3.49)":
