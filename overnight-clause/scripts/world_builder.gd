...
## 403 — Dane's flat. A re-dress of the 404 geometry rather than a new build: same
## room shell and the same furniture calls, mirrored across the shared bedroom wall,
## with warm dressing (a lamp, unpacked boxes, a made bed) instead of abandoned
## dressing. Dark and unlit until a scene puts the player on the other side of the wall.
var _403_lamp: OmniLight3D


func _build_403() -> void:
	var c := Vector3(4.45, 0, 12.6)
	_room_shell(c, Vector2(6.4, 3.7), _floor_mat)
	_box(Vector3(1.2, 1.25, c.z), Vector3(0.16, 2.6, 3.7), _plaster)
	_box(Vector3(7.7, 1.25, c.z), Vector3(0.16, 2.6, 3.7), _plaster)
	_box(Vector3(c.x, 1.25, 14.45), Vector3(6.5, 2.6, 0.16), _plaster)
	# mirrored: bed against the shared wall, wardrobe on the opposite side to 404's
	_bed(Vector3(5.5, 0.22, 12.2))
	_table(Vector3(6.9, 0.42, 13.6))
	_counter(Vector3(2.1, 0.45, 13.4))
	# two weeks of not unpacking, and two weeks of not sleeping
	for i in 6:
		_box(Vector3(2.6 + (i % 3) * 0.62, 0.22 + float(i / 3) * 0.44, 11.6), Vector3(0.56, 0.42, 0.44), _trim)
	for i in 4:
		_box(Vector3(6.6 + (i % 2) * 0.12, 0.46 + i * 0.03, 13.4 + i * 0.08), Vector3(0.16, 0.03, 0.2), _paper, false)
	# the service cavity mouth on this side, which is how the player gets here
	_box(Vector3(7.62, 0.42, 12.9), Vector3(0.1, 0.7, 0.72), _dark, false)
	_403_lamp = OmniLight3D.new()
	_403_lamp.light_color = Color(1.0, 0.78, 0.5)
	_403_lamp.light_energy = 0.0
	_403_lamp.omni_range = 5.5
	_403_lamp.position = Vector3(6.9, 1.1, 13.5)
	add_child(_403_lamp)
	_zone_label(Vector3(4.5, 2.42, 14.3), "zone.6")


## Called when a scene moves the fiction into 403: the only light in the flat comes up,
## and the corridor outside goes the colour the refused-clause corridor goes.
func reveal_403(cold_corridor: bool) -> void:
	if _403_lamp:
		_403_lamp.light_energy = 1.35
	if cold_corridor:
		for child in get_children():
			if child is Light3D and (child as Node3D).position.z < 10.0:
				(child as Light3D).light_color = Color(0.16, 0.22, 0.42)
