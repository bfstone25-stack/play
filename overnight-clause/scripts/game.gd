...
## Web exports built with the `slice` feature tag stop with the stain choice made,
## the mirror plate seen and the pipe knocking — one beat before the first gated
## plate (cg_hatch). In the base game this constant was 2; moving the free/paid
## boundary in this fork is this one line, which is the single best piece of luck
## in the whole project.
static var SLICE: bool = OS.has_feature("slice")
const SLICE_LAST_STAGE := 5
...
func _stage_items(s: int) -> Array:
	return Overnight.stage(s, flags)


func _commentary(id: String) -> String:
	return Overnight.commentary(id)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo and event.physical_keycode == KEY_G and not hud.is_vn_open() and not hud.is_choice_open():
		hud.toggle_gallery()
		return
	if hud and hud.is_gallery_open():
...
## Play one authored plate moment: a full-screen picture under a full-screen scene.
## Every call site below is an existing flag write, which is the whole design — the fork
## adds a plate(id) call next to a line that already ran.
func plate_scene(plate_id: String, scene_id: String, cont: Callable, lead := "") -> void:
	player.locked = true
	Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
	var text := OvernightScript.scene(scene_id)
	if lead != "":
		text = lead + "\n---\n" + text
	hud.show_story(scene_id, text, true, func() -> void:
		hud.hide_plate()
		if not ending:
			player.locked = false
			player.capture_mouse()
	...
	if scene_id in ["ch4_hatch", "ch6_403"] and $World.has_method("reveal_403"):
		$World.reveal_403(scene_id == "ch6_403")
	if plate_id != "":
...
func on_note(id: String) -> void:
	...
	match id:
		...
		"dane":
			...
			hud.set_objective(Loc.t(objective_key))
			# game.gd:334 in the base. Dane stops being a voice under a door.
			plate_scene("", "ch2_dane", Callable())
		"fire_plan":
		...
		"cassette":
			...
			# base game.gd:364. cg_cavity is never censored and never gated: it is the
			# frightened plate, and it has to be free.
			plate_scene("cg_cavity", "ch5_cavity", func() -> void:
...
func _resolve_choice(choice_id: String, i: int, source: Node) -> void:
	...
	match choice_id:
		"stain":
			# base game.gd:393/396. Either branch: the leak soaks her and the mirror
			# plate follows. Free in every build — tone-setter and store screenshot.
			var lead := Loc.t("note.stain_keep") if i == 0 else Loc.t("note.stain_wipe")
			if i == 0:
			...
			return
		"pipe":
			# base game.gd:401. Answering the pipe is what brings Dane through the wall,
			# so the scene is literally paid for by believing the tenant.
			var pipe_lead := Loc.t("note.pipe_yes") if i == 0 else Loc.t("note.pipe_no")
			var pipe_scene := "ch4_hatch" if i == 0 else "ch4_silent"
			var pipe_plate := "cg_hatch" if i == 0 else ""
			if i == 0:
			...
			return
		"clause":
			# base game.gd:409/412. cg_403 is the three-flag plate: refusing the clause,
			# having answered the pipe and picked up his note at :334.
			var clause_lead := Loc.t("note.clause_yes") if i == 0 else Loc.t("note.clause_no")
			var clause_scene := ""
			var clause_plate := ""
			if i == 0:
				...
				if flags["pipe_answered"] and flags["dane_note"]:
					clause_scene = "ch6_403"
					clause_plate = "cg_403"
			if clause_scene == "":
				hud.show_note(clause_lead)
				_advance(11, "obj.clause", 6, "02:27")
			else:
				plate_scene(clause_plate, clause_scene, func() -> void:
					_advance(11, "obj.clause", 6, "02:27")
				, clause_lead)
			return
		"final":
...
func _finish(id: String) -> void:
	...
	# base game.gd:453-458 resolver output -> one ending plate each, and the fork's
	# ending prelude as the first beat. Iris appears only in cg_witness and only as the
	# bible specifies her: wet coat silhouette behind plastic, no facial features.
	...
	match id:
		"WITNESS":
			...
			hud.show_plate("cg_witness")
		"COMPLICIT":
			hud.show_ending(Loc.t("end.complicit"), [
				OvernightScript.scene("ch7_complicit"),
				Loc.t("beat.complicit.0"), Loc.t("beat.complicit.1"), Loc.t("beat.complicit.2"), Loc.t("beat.complicit.3")])
			hud.show_plate("cg_complicit")
		_:
			hud.show_ending(Loc.t("end.404"), [
				OvernightScript.scene("ch7_404"),
				Loc.t("beat.404.0"), Loc.t("beat.404.1"), Loc.t("beat.404.2"), Loc.t("beat.404.3")])
			hud.show_plate("cg_404")
