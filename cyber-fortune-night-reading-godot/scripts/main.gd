extends Control
## Main — the shell of the night fork: three screens (the table, a reading, the night),
## the HUD over them, toasts, and the web dev bridge's UI commands. Portrait 720x1280.
##
## The parent's six screens (tube / deck / reader / rack / shop) are the day game and are
## not carried over: this world's instruments live inside the reading, because here the
...
## The adult catalogue, not the casual one: this build only ever runs on the adult host
## (ops/check_adsense_isolation.py proves the other direction), and board.js refuses to
## draw the adult board anywhere it is not allowed.
...
func _bridge(cmd: Dictionary) -> Dictionary:
	...
	if op == "ui_state":
		# Night's own state rides along with the UI state, so one bridge read tells the
		# driver which screen is up AND where every woman stands.
		var s := {"screen": current_name}
		s.merge(Night.dev_state(), true)
