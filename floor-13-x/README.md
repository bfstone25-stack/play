> **This is the adult (18+) fork, `floor-13-retention`.** The document below is the
> mainstream game's and still describes the base design accurately. What the fork
> changes on top of it is in [FORK.md](./FORK.md) — read that first.

# Floor 13: Night Shift

Complete standalone Godot 4.7.2 pixel-horror interactive narrative.

- Canonical script and production contract: [`PRODUCT_BIBLE.md`](./PRODUCT_BIBLE.md)
- Main scene: `scenes/main.tscn`
- Runtime: 7 chapters, 28 point-click hotspots, 4 persistent decisions, 3 endings
- Input: mouse/touch only; no typing or movement controls
- Presentation: bottom ADV bar for routine talk/hotspots; full-screen NVL (red filter + shake) for diary, collapse, and endings
- Tests: `tests/routes.gd`, `tests/playthroughs.gd`, `tests/smoke.gd`, `tests/first_read.gd`, and `tests/vn_chrome.gd`
- Web route: `/floor-13/`

The former Beat Monday browser fighter remains under `frontend/` as an excluded historical archive. It is not loaded or exported by this game.
